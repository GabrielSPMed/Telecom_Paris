...
public class Dictionary extends Observable {
	...
	public boolean isFeasible() {
		...
		//Simplex.output.println("Method isFeasible to write");
		//incomplete = true;
		int i;
		for (i=1; i <=this.nbBasicVar; i++) {
			if (this.D[i][0]<0) return false;
		}
		// To modify
		return true; 
	}
	...
	// Two following lines to delete
		//Simplex.output.println("Method searchFirstIndexEnteringVariable to write");
		//incomplete = true;
	int i;
	for (i=1;i<=this.nbNonBasicVar;i++) {
		if (this.D[0][i]>0) return i;
	}
		// To modify
		return 0; 
	}
	...
	public int searchIndexLeavingVariable(int jE)  {
		// Three following lines to delete	
		//Simplex.output.println("Method searchIndexLeavingVariable to write");
		//incomplete = true;
		
		double aux_value = Double.MAX_VALUE;
		int aux_index = 0;
		int i;
		for (i=1;i<=this.nbBasicVar;i++) {
			if (this.D[i][jE]< 0) {
				if((-this.D[i][0]/this.D[i][jE])< aux_value) {
					aux_value = -this.D[i][0]/this.D[i][jE];
					aux_index = i;
				}
			}
		}
	...
	public void pivote(int iS, int jE) {
		double auxArray[];
		auxArray = new double[this.nbNonBasicVar + 1];
		
		//Give aux array the line that contains iS and jE
		//Aux: 30 -5 -3
		
		int i;
		for (i=0;i<=this.nbNonBasicVar;i++) {
			auxArray[i]= this.D[iS][i];
		}
		
		//Give aux array -1 in the intersection between iS and jE
		//Aux: 30 -1 -3
		auxArray[jE] = -1;
		
		//Divide aux array by - the original coefficient in iSjE
		//Aux: 6 -0.2 -0.6
		for (i=0;i<=this.nbNonBasicVar;i++) {
			auxArray[i] = - auxArray[i]/this.D[iS][jE];
		
		}
		
		//Fill the iS line with zeros
		/* 0   4  3
		 * 24 -2 -3
		 * 0   0  0
		 * 18 -1 -3
		...
		//Multiply auxArray by the jE coefficient for each line
		//Set it to zero in D
		//Add the array to each line
		int j;
		double auxFactor;
		for (i=0;i<=this.nbBasicVar;i++) {
			auxFactor = this.D[i][jE];
			if(i==iS) auxFactor=1;
			this.D[i][jE] = 0;
			for (j=0;j<=this.nbNonBasicVar;j++) {
				this.D[i][j]=this.D[i][j]+auxFactor*auxArray[j];
			}
		}
		//Swapping the basis arrays
		int auxLeaving;
		auxLeaving = this.arrayBasicVar[iS];
		this.arrayBasicVar[iS] = this.arrayNonBasicVar[jE];
		this.arrayNonBasicVar[jE] = auxLeaving;
	...
	public int searchEnteringAdvantageousVariableIndex() {	
		...
		double auxVal = 0;
		for (i=1;i<=this.nbNonBasicVar;i++) {
			auxj = this.searchIndexLeavingVariable(i);
			if (this.D[auxj][i] == 0) return -1;
			if (-this.D[auxj][0]*this.D[0][i]/this.D[auxj][i]>auxVal) {
				auxVal = -this.D[auxj][0]*this.D[0][i]/this.D[auxj][i];
				auxi = i;
			}
		}
	...
	public int searchIndexEnteringVariableSmallestNumber(){		
		// Two following lines to delete
		double auxValue = Double.MAX_VALUE;
		int i, auxi = 0;
		for (i = 1; i<=this.nbNonBasicVar;i++) {
			if (this.D[0][i]<auxValue && this.D[0][i] > 0) {
				auxValue = this.D[0][i];
				auxi = i;
			}
		}
	...
	public int searchIndexLeavingVariableSmallestNumber(int jE) {		
		// Two following lines to delete
		int i, auxi = 0, isDegenerate = 0;
		double auxValue = Double.MAX_VALUE;
		// To modify
		for (i=1;i<=this.nbBasicVar;i++) {
			if (this.D[i][0]==0 && this.D[i][jE]<auxValue) {
				isDegenerate = 1;
				auxValue = this.D[i][jE];
				auxi = i;
			}
		}
		if (isDegenerate==1) return auxi;
		return this.searchIndexLeavingVariable(jE);
